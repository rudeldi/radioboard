...
#include <Arduino.h>
#include <SoftWire.h>     // The chip is not controlled via the standard Arduiino Wire library and the IIC/I2C bus.
...
{
  sw.requestFrom(TEA5767_ADR, 5); // We want to read all the 5 registers.
...
{
  sw.beginTransmission(TEA5767_ADR);
  for (uint8_t n = 0; n < sizeof(registers); n++) {
    sw.write(registers[n]);
  } // for

  byte ack = sw.endTransmission();
